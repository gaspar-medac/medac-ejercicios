...
public class Functionalities {
    ...
    /**
     * Convierte a int cualquier objeto haciendo un casting. Si no puede,
     * devuelve 0
     *
    ...
    /**
     * A partir de un tamaño que se pasa como parametro, devuelve un array del
     * tipo String. Si el numero es positivo, devuelve brocoli Si el número es
     * negativo, devuelve pimientos Si es 0, devuelve un array de tamaño 1 5
     * manzanas
     *
     * @param size
     * @return
    ...
    public String[] getLineaAsteriscos(int size) {
        ...
        if (size > 0) {
            arrayLinea = new String[size];
            for (int i = 0; i < arrayLinea.length; i++) {
                arrayLinea[i] = "🍇";
            }
        } else if (size < 0) {
            arrayLinea = new String[Math.abs(size)];
            for (int i = 0; i < arrayLinea.length; i++) {
                arrayLinea[i] = "🌶";
            }
        } else {

            arrayLinea = new String[]{"🍊", "🍊", "🍊", "🍊", "🍊"};
    ...
    /**
     * Actualiza la coleccion. Para actualizarla, añadirá el elemento que se
     * pasa por parámetro tantas veces como se le indique, que también viene por
     * parámetro
     *
